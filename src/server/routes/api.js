...
const { getUserByLogin, getChannelFollowers } = require('../../twitch/helixClient');
...
router.get('/twitch/status', async (req, res) => {
  ...
  const followersResult = await getChannelFollowers(CHANNEL, token.broadcasterId, 1);

  res.json({
    connected: true,
    broadcasterId: token.broadcasterId,
    login: userInfo?.login || CHANNEL,
    displayName: userInfo?.display_name || CHANNEL,
    profileImageUrl: userInfo?.profile_image_url || null,
    followerCount: followersResult?.total ?? null
  });
});

// Déconnecte uniquement le compte Twitch (retire le token) : reste connecté au dashboard.
// Les alertes/EventSub/setgame ne fonctionneront plus tant que "Connecter Twitch" n'est pas refait.
router.post('/twitch/disconnect', async (req, res) => {
  await TwitchToken.findOneAndDelete({ channel: CHANNEL });
  res.json({ ok: true });
});
